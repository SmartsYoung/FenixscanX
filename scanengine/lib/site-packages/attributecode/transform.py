...
def normalize_dict_data(data):
    """
    Check if the input data from scanengine-toolkit and normalize to a normal
    dictionary if it is.
    Return a list type of normalized dictionary.
    """
    try:
        # Check if this is a JSON output from scanengine-toolkit
        if(data["headers"][0]["tool_name"] == "scanengine-toolkit"):
