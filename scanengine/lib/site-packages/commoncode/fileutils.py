...
try:
    from scanengine_config import scanengine_temp_dir as _base_temp_dir
except ImportError:
    _base_temp_dir = None
...
def get_temp_dir(base_dir=_base_temp_dir, prefix=''):
    ...
    If `base_dir` is not provided, use the 'SCANENGINE_TMP' env var or the system
    temp directory.

    WARNING: do not change this code without changing scanengine_config.py too
    """

    has_base = bool(base_dir)
    if not has_base:
        base_dir = os.getenv('SCANENGINE_TMP')
        if not base_dir:
