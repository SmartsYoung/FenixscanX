...
scanengine_root_dir = path.dirname(
    # etc
    path.dirname(
        # conf
        path.dirname(
...
def setup_dev_mode():
    """
    Create the development mode tag file. In development mode, ScanEngine does
    ...
    """
    with open(path.join(scanengine_root_dir, 'SCANENGINE_DEV_MODE'), 'w') as sdm:
        sdm.write('This is a tag file to notify that ScanEngine is used in development mode.')
...
def setup_vscode():
    ...
    source = path.join(scanengine_root_dir, 'etc', settings)
    vscode = path.join(scanengine_root_dir, '.vscode')
    target = path.join(scanengine_root_dir, settings)
