...
def add_osi_key(obj: "OSI License Object"):
    ...
    # find matching Scanengine License Object for `obj`
    result = find_osi_map(obj) 
    # derive OSI License Key from `obj`
    osi_key = obj['id'] 
    if not result: # no matching Scanengine License found
        return None 
    mapped_license = licenses[result] # license object from Scanengine
...
def add_osi_keys():
    """
    For every possible OSI License, match and add OSI Key to existing Scanengine License
    """
    #retrieve OSI License Objects
    contents = urllib.request.urlopen("https://api.opensource.org/licenses/").read().decode()
    data = json.loads(contents)
    # for each OSI License Object, update matching Scanengine License Object
    for obj in data:
        add_osi_key(obj)
