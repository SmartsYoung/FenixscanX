...
"""
Sync and update the ScanEngine licenses against:
...
class ScanEngineLicenses(object):
    """
    Licenses from the current ScanEngine installation
    """
...
class ExternalLicensesSource(object):
    ...
    # between external and scanengine: one of "key" or "spdx_license_key"
    key_attribute = None

    # tuple of ScanEngine reference license attributes that can be updated
    # from this source
    updatable_attributes = tuple()

    # tuple of ScanEngine reference license attributes that cannot be updated
    ...
    def get_licenses(self, scanengine_licenses):
        """
        Return a mapping of key -> ScanEngine License objects either fetched
        ...
        licenses = []
        for lic, text in self.fetch_licenses(scanengine_licenses):
            try:
    ...
    def fetch_licenses(self, scanengine_licenses):
        """
...
def get_key_through_text_match(key, text, scanengine_licenses, match_approx=False):
    """
...
class SpdxSource(ExternalLicensesSource):
    ...
    def fetch_licenses(self, scanengine_licenses, from_repo=SPDX_DEFAULT_REPO):
        """
        ...
        with zipfile.ZipFile(licenses_zip) as archive:
            for path in archive.namelist():
                ...
                if path.endswith('+.json'):
                    ...
                    # ScanEngine, but they are deprecated in SPDX.
                    continue
                details = json.loads(archive.read(path), object_pairs_hook=OrderedDict)
                lic = self.build_license(details, scanengine_licenses)
                if lic:
                    yield lic

    def build_license(self, mapping, scanengine_licenses):
        """
        Return a ScanEngine License object built from an SPDX license mapping.
        """
        ...
        # TODO: Not yet available in ScanEngine
        is_foreign = key in scanengine_licenses.non_english_by_spdx_key
        if is_foreign:
        ...
        lic = License(
            ...
            # FIXME: available in ScanEngine but as an OSI URL
...
class DejaSource(ExternalLicensesSource):
    ...
    def fetch_licenses(self, scanengine_licenses):
        ...
        for licenses in call_deja_api(api_url, self.api_key, paginate=100):
            for lic in licenses:
                dlic = self.build_license(lic, scanengine_licenses)
                if dlic:
                    yield dlic

    def build_license(self, mapping, scanengine_licenses):
        """
        Return a ScanEngine License object built from a DejaCode license
        mapping or None for skipped licenses.
        """
        key = mapping['key']

        # TODO: Not yet available in ScanEngine
        is_foreign = key in scanengine_licenses.non_english_by_key
        if is_foreign:
        ...
        if deprecated and key not in scanengine_licenses.by_key:
            if TRACE: print('Skipping deprecated license not in ScanEngine:', key)
            return
...
def merge_licenses(scanengine_license, external_license, updatable_attributes,
                   from_spdx=False):
    """
    Compare and update two License objects in-place given a sequence of
    `updatable_attributes`.
    Return a two-tuple of lists as:
        (scanengine license updates, other license updates)
    ...
    matching. In this case, the key that is used is that from the ScanEngine
    license.
    """
    if TRACE:
        print('merge_licenses:', scanengine_license, external_license)

    updated_scanengine_attributes = []

    def update_scanengine(_attrib, _sc_val, _ext_val):
        setattr(scanengine_license, _attrib, _ext_val)
        updated_scanengine_attributes.append((_attrib, _sc_val, _ext_val))
    ...
    if from_spdx:
        scanengine_key = scanengine_license.spdx_license_key
        external_key = external_license.spdx_license_key
        if scanengine_key != external_key:
            raise Exception('Non mergeable licenses with different SPDX keys: %(scanengine_key)s <> %(external_key)s' % locals())
    else:
        scanengine_key = scanengine_license.key
        external_key = external_license.key
        if scanengine_key != external_key:
            raise Exception('Non mergeable licenses with different keys: %(scanengine_key)s <> %(external_key)s' % locals())

#         if scanengine_license.spdx_license_key != external_license.spdx_license_key:
#             pass
#         else:
#             if TRACE:
#                 print('Merging licenses with different keys, but same SPDX key: %(scanengine_key)s <> %(external_key)s' % locals())
#             update_external('key', scanengine_key, external_key)

    for attrib in updatable_attributes:
        scanengine_value = getattr(scanengine_license, attrib)
        ...
        if isinstance(scanengine_value, bool) and isinstance(external_value, bool):
            if scanengine_value is False and scanengine_value != external_value:
                update_scanengine(attrib, scanengine_value, external_value)
            continue

        if isinstance(scanengine_value, (list, tuple)) and isinstance(external_value, (list, tuple)):
            normalized_scanengine_value = set(s for s in scanengine_value if s and s.strip())
            ...
            if attrib.endswith('_urls',):
                all_sc_urls = set(
                    list(normalized_scanengine_value) +
                    scanengine_license.text_urls +
                    scanengine_license.other_urls +
                    [scanengine_license.homepage_url,
                     scanengine_license.osi_url,
                     scanengine_license.faq_url
                    ]
                )
                all_sc_urls = set(u for u in all_sc_urls if u)
                new_other_urls = normalize_external_value.difference(all_sc_urls)
                # add other urls to ScanEngine
                combined = normalized_scanengine_value | new_other_urls
                if set(normalized_scanengine_value) != combined:
                    update_scanengine(attrib, scanengine_value, sorted(combined))

                # FIXME: FOR NOW WE DO NOT UPDATE THE OTHER SIDE with ScanEngine URLs

            else:
                # merge ScanEngine and other value lists
                combined = normalized_scanengine_value | normalize_external_value
                if combined == normalized_scanengine_value:
                    pass
                else:
                    update_scanengine(attrib, scanengine_value, sorted(combined))
                # FIXME: FOR NOW WE DO NOT UPDATE THE OTHER SIDE with ScanEngine seqs

            continue

        if (isinstance(scanengine_value, compat.unicode) and isinstance(external_value, compat.unicode)):
            # keep the stripped and normalized spaces value
            # normalized spaces
            normalized_scanengine_value = ' '.join(scanengine_value.split())
            normalize_external_value = ' '.join(external_value.split())

            # Fix up values with normalized values
            if scanengine_value != normalized_scanengine_value:
                scanengine_value = normalized_scanengine_value
                update_scanengine(attrib, scanengine_value, normalized_scanengine_value)
        ...
        scanengine_equals_external = scanengine_value == external_value
        if scanengine_equals_external:
            continue

        external_is_empty = scanengine_value and not external_value
        if external_is_empty:
            update_external(attrib, scanengine_value, external_value)
            continue

        scanengine_is_empty = not scanengine_value and external_value
        if scanengine_is_empty:
            update_scanengine(attrib, scanengine_value, external_value)
            continue

        # on difference, the other license wins
        if scanengine_value != external_value:
            update_scanengine(attrib, scanengine_value, external_value)
            continue

    return updated_scanengine_attributes, updated_external_attributes


def synchronize_licenses(scanengine_licenses, external_source, use_spdx_key=False,
                         match_text=False, match_approx=False):
    """
    Update the `scanengine_licenses` ScanEngineLicenses licenses and texts in-place
    ...
    - new licenses found in scanengine that are not in the external source are created in the new sub dir
    - external licenses that are modified from Scanengine are created in the updated sub dir
    - external licenses that are not found in Scanengine are created in the deleted sub dir

    The process is this:
    fetch external remote licenses
    build external license objects in memory, save in the original sub directory
    load scanengine licenses

    for each scanengine license:
    ...
    for each external license:
        find a possible exact key or spdx key match with a scanengine license
        if there is a match, update and save the scanengine license object
        if there is no match, create and save a new scanengine license object
    ...
    scanengines_by_key = scanengine_licenses.by_key
    externals_by_key = external_source.get_licenses(scanengine_licenses)

    if use_spdx_key:
        scanengines_by_key = scanengine_licenses.by_spdx_key
    ...
    same = set()
    added_to_scanengine = set()
    added_to_external = set()
    updated_in_scanengine = set()
    updated_in_external = set()

    unmatched_scanengine_by_key = {}

    # FIXME: track deprecated
    # removed = set()

    # 1. iterate scanengine licenses and compare with other
    for matching_key, scanengine_license in scanengines_by_key.items():

        if not TRACE:print('.', end='')

        # does this scanengine license exists in others based on the matching key?
        external_license = externals_by_key.get(matching_key)
        if not external_license:
            if TRACE_DEEP: print('ScanEngine license not in External:', matching_key)
            unmatched_scanengine_by_key[scanengine_license.key] = scanengine_license
            continue

        # the matching key exists on both sides: merge/update both licenses
        scanengine_updated, external_updated = merge_licenses(
            scanengine_license, external_license,
            external_source.updatable_attributes,
            from_spdx=use_spdx_key)

        if not scanengine_updated and not external_updated:
            if TRACE_DEEP: print('License attributes are identical:', matching_key)
            same.add(matching_key)

        if scanengine_updated:
            if TRACE: print('ScanEngine license updated: SPDX:', use_spdx_key, matching_key, end='. Attributes: ')
            for attrib, oldv, newv in scanengine_updated:
                if TRACE: print('  %(attrib)s: %(oldv)r -> %(newv)r' % locals())
            updated_in_scanengine.add(matching_key)
    ...
    """
        if not external_license:
            matched_key = get_key_through_text_match(
                matching_key, scanengine_license.text,
                scanengine_licenses,
                match_approx=True)
            if matched_key:
                print('\nScanEngine license not in External:', matching_key, 'but matched to:', matched_key)
                external_license
            else:
                print('\nScanEngine license not in External:', matching_key, ' and added to external')
                external_license = scanengine_license.relocate(external_source.new_dir)
                added_to_external.add(matching_key)
                externals_by_key[matching_key] = external_license
                continue
"""
    # 2. iterate other licenses and compare with ScanEngine
    if TRACE: print()
    for matching_key, external_license in externals_by_key.items():
        # does this key exists in scanengine?
        scanengine_license = scanengines_by_key.get(matching_key)
        if scanengine_license:
        ...
        if match_text:
            ...
            if TRACE:
                print('External license with different key:', matching_key, 'and text matched to ScanEngine key:', matched_key)

            if matched_key:
                print('\nExternal license with different key and matched text to ScanEngine:', matching_key, ' matched to:', matched_key)
                if matched_key in unmatched_scanengine_by_key:
                    del unmatched_scanengine_by_key[matched_key]

                scanengine_license = scanengines_by_key.get(matched_key)
            ...
            # Create a new ScanEngine license
            scanengine_license = external_license.relocate(licensedcode.models.licenses_data_dir, matching_key)
            added_to_scanengine.add(matching_key)
            scanengines_by_key[matching_key] = scanengine_license
            if TRACE: print('External license key not in ScanEngine:', matching_key, 'created in ScanEngine.', 'SPDX:', use_spdx_key)

    # 3. For scanengine licenses that were not matched to anything in external add them in external
    if TRACE:
        print()
        print('Processing unmatched_scanengine_by_key.')
    for lkey, scanengine_license in unmatched_scanengine_by_key.items():
        if lkey in set(['here-proprietary']):
            continue
        if scanengine_license.is_deprecated:
            continue
        external_license = scanengine_license.relocate(external_source.new_dir)
        added_to_external.add(lkey)
        externals_by_key[lkey] = external_license
        if TRACE: print('ScanEngine license key not in External:', lkey, 'created in External.')

    # finally write changes in place for updates and news
    for k in updated_in_scanengine | added_to_scanengine:
        scanengines_by_key[k].dump()
...
# for addition
    ...
    print('Add to ScanEngine:     ', len(added_to_scanengine))
    print('Updated in ScanEngine: ', len(updated_in_scanengine))
...
@click.option('-m', '--match-text', is_flag=True, default=False, help='Match external license texts with license detection to find a matching ScanEngine license.')
@click.option('-a', '--match-approx', is_flag=True, default=False, help='Include approximate license detection matches when matching ScanEngine license.')
...
def cli(license_dir, source, match_text, match_approx, trace, create_ext):
    """
    Synchronize ScanEngine licenses with an external license source.

    DIR is the directory to store fetched external licenses (or where to load from already fetched licenses).
    Side-by-side with "DIR", three directories are created with new, updated or deleted licenses (with regards to ScanEngine licenses)
    The ScanEngine licenses are collected from the current installation.
    ...
    scanengine_licenses = ScanEngineLicenses()

    use_spdx_key = source == 'spdx'
    added_to_external = synchronize_licenses(scanengine_licenses, external_source,
