...
def _patch_license_list():
    """
    Patch the SPDX library license list to match the list of ScanEngine known SPDX
    licenses.
    """
...
def write_spdx(output_file, files, tool_name, tool_version, notice, input_file, as_tagvalue=True):
    ...
    doc.comment = notice
    tool_name = tool_name or 'ScanEngine'
        ...
        if file_licenses:
            ...
            for file_license in file_licenses:
                ...
                if spdx_id:
                    ...
                    licenseref_id = 'LicenseRef-scanengine-' + license_key
    ...
    # The file passed by ScanEngine for output is opened in text mode Therefore in
