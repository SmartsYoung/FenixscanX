...
def get_emails(location, threshold=50, test_slow_mode=False, test_error_mode=False, **kwargs):
    ...
    """
    if test_error_mode:
        raise ScanengineError('Triggered email failure')
...
SCANENGINE_DEBUG_PACKAGE_API = os.environ.get('SCANENGINE_DEBUG_PACKAGE_API', False)
...
def get_package_info(location, **kwargs):
    ...
    try:
        ...
        if SCANENGINE_DEBUG_PACKAGE_API:
            raise
        else:
