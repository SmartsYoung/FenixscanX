...
from scanengine.pool import get_pool

# Import early because of the side effects
import scanengine_config
...
from scanengine import ScanengineError
from scanengine import ScanengineCliUsageError
from scanengine import notice
from scanengine import print_about
from scanengine import Scanner
from scanengine.help import epilog_text
from scanengine.help import examples_text
from scanengine.interrupt import DEFAULT_TIMEOUT
from scanengine.interrupt import fake_interruptible
from scanengine.interrupt import interruptible
...
def print_version(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    click.echo('ScanEngine version ' + scanengine_config.__version__)
    ctx.exit()


class ScanengineCommand(GroupedHelpCommand):
    short_usage_help = '''
Try the 'scanengine --help' option for help on options and arguments.'''
...
try:
    ...
    echo_stderr('========================================================================')
    echo_stderr('ERROR: Unable to import ScanEngine plugins.'.upper())
    echo_stderr('Check your installation configuration (setup.py) or re-install/re-configure ScanEngine.')
...
def print_plugins(ctx, param, value):
    ...
    for plugin_cls in sorted(plugin_classes, key=lambda pc: (pc.stage, pc.name)):
        click.echo('--------------------------------------------')
        click.echo('Plugin: scanengine_{self.stage}:{self.name}'.format(self=plugin_cls), nl=False)
...
@click.command(name='scanengine',
    epilog=epilog_text,
    cls=ScanengineCommand,
...
@click.option('--about',
    ...
    help='Show information about ScanEngine and licensing and exit.',
...
@click.option('--plugins',
    ...
    help='Show the list of available ScanEngine plugins and exit.',
...
@click.option('--test-mode',
    ...
    hidden=True,
    help='Run ScanEngine in a special "test mode". Only for testing.',
...
@click.option('--test-slow-mode',
    ...
    # not yet supported in Click 6.7 but added in PluggableCommandLineOption
    hidden=True,
    help='Run ScanEngine in a special "test slow mode" to ensure that --email scan needs at least one second to complete. Only for testing.',
    help_group=cliutils.MISC_GROUP, sort_order=1000, cls=PluggableCommandLineOption)
...
@click.option('--test-error-mode',
    ...
    # not yet supported in Click 6.7 but added in PluggableCommandLineOption
    hidden=True,
    help='Run ScanEngine in a special "test error mode" to trigger errors with the --email scan. Only for testing.',
    help_group=cliutils.MISC_GROUP, sort_order=1000, cls=PluggableCommandLineOption)
...
def scanengine(ctx, input,  # NOQA
    ...
    """
    This function is the main ScanEngine CLI entry point.
    ...
    try:
        ...
        if TRACE_DEEP:
            logger_debug('scanengine: ctx.params:')
            for co in sorted(ctx.params.items()):
                logger_debug('  scanengine: ctx.params:', co)
        ...
        # check for updates
        from scanengine.outdated import check_scanengine_version
        outdated = check_scanengine_version()
...
def run_scan(
    ...
    results but as native Python. Raise Exceptions (e.g. ScanengineError) on
    error. See scanengine() for arguments details.
    """
    ...
    try:
        ...
        if from_json and scanner_plugins:
            ...
            raise ScanengineCliUsageError(msg)
        ...
        if TRACE_DEEP:
            logger_debug('scanengine: all_enabled_plugins_by_qname:', all_enabled_plugins_by_qname)
            logger_debug('scanengine: non_enabled_plugins_by_qname:', non_enabled_plugins_by_qname)
        ...
        if TRACE_DEEP:
            logger_debug('scanengine: plugins_to_setup: from required:', plugins_to_setup)

        plugins_to_setup.extend(all_enabled_plugins_by_qname.values())

        if TRACE_DEEP:
            logger_debug('scanengine: plugins_to_setup: includng enabled:', plugins_to_setup)
        ...
        if TRACE_DEEP:
            logger_debug('scanengine:resource_attributes')
        ...
        if TRACE_DEEP:
            logger_debug('scanengine:codebase_attributes')
        ...
        if not quiet:
            echo_func('Collect file inventory...(获取文件目录...)', fg='green')
        ...
        cle.tool_name = 'scanengine-toolkit'
        cle.tool_version = scanengine_config.__version__
        cle.notice = notice
        ...
        if not quiet:
            ...
            echo_func('Scanning done.(扫描结束)', fg='green' if success else 'red')
        ...
        scanengine_temp_dir = scanengine_config.scanengine_temp_dir
        if keep_temp_files:
            if not quiet:
                msg = 'Keeping temporary files in: "{}".'.format(scanengine_temp_dir)
                echo_func(msg, fg='green' if success else 'red')
        else:
            if not quiet:
                echo_func('Removing temporary files...(清理临时文件...)', fg='green', nl=False)

            from commoncode import fileutils
            fileutils.delete(scanengine_temp_dir)
...
def get_displayable_summary(codebase, scan_names, processes, errors):
    ...
    summary_messages = []
    summary_messages.append('Summary(汇总结果):        %(scan_names)s with %(processes)d process(es)' % locals())
    summary_messages.append('Errors count(扫描错误):   %(errors_count)d' % locals())
    summary_messages.append('Scan Speed(扫描速度):     %(scan_file_speed).2f files/sec. %(scan_size_speed)s' % locals())
    if prescan_scan_time:
        summary_messages.append('Early Scanners Speed:     %(prescan_scan_file_speed).2f '
                    'files/sec. %(prescan_scan_size_speed)s' % locals())

    summary_messages.append('Initial counts(初始文件数): %(initial_res_count)d resource(s): '
                               '%(initial_files_count)d file(s) '
                               'and %(initial_dirs_count)d directorie(s) '
                               '%(initial_size_count)s' % locals())

    summary_messages.append('Final counts(去重后文件数):   %(final_res_count)d resource(s): '
                               '%(final_files_count)d file(s) '
                               'and %(final_dirs_count)d directorie(s) '
                               '%(final_size_count)s' % locals())

    summary_messages.append('Timings(耗时):')

    cle = codebase.get_or_create_current_header().to_dict()
    summary_messages.append('  scan_start(起始时间): {start_timestamp}'.format(**cle))
    summary_messages.append('  scan_end(结束时间):   {end_timestamp}'.format(**cle))
