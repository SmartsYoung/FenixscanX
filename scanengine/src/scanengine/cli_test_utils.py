...
from scanengine_config import scanengine_root_dir
...
def run_scan_click(options, monkeypatch=None, test_mode=True, expected_rc=0, env=None, retry=on_windows):
    ...
    result = runner.invoke(cli.scanengine, options, catch_exceptions=False, env=env)
...
def remove_windows_extra_timeout(scanengine_options, timeout=WINDOWS_CI_TIMEOUT):
    """
    Strip a test timeout from a pretty scanengine_options mapping if on Windows.
    """
    if on_windows:
        if scanengine_options and scanengine_options.get('--timeout') == timeout:
            del scanengine_options['--timeout']
